...
async def cmd_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "🎯 *Сигнал дня* — сканирует ТОП-50 монет, выбирает 1 лучший\n"
...
def _format_signal_teaser(sig) -> str:
    """Урезанная версия для бесплатных пользователей."""
...
# ════════════════════════════════════════════════════════════
#  🎯 СИГНАЛ ДНЯ — НОВАЯ ЛОГИКА
#  Сканирует ТОП-50 монет, выбирает 1 лучшую сделку
# ════════════════════════════════════════════════════════════

async def _show_signals(chat_id: int):
    level = get_access_level(chat_id)

    # ── Бесплатный: лимит проверка ──────────────────────
    if level == "free":
    ...
    # ── Статус-сообщение (редактируемое) ────────────────
    ...
    try:
        from signal_engine import (
            ...
            format_no_signal, save_signals, Signal,
        )
        from news_fetcher import fetch_news

        # ── Параллельно: скан + новости ─────────────────
        signal_task = asyncio.create_task(
            scan_market_for_best_signal(
        ...
        # ── Нет сигнала ─────────────────────────────────
        if not best_signal:
            no_sig_text = format_no_signal(scanned)
            if news_text:
                no_sig_text = news_text + "\n\n" + no_sig_text
            await safe_send(chat_id, no_sig_text, reply_markup=main_keyboard())
            return

        # ── Полный доступ ────────────────────────────────
        if level in ("admin", "paid", "trial"):
            # AI комментарий (опционально)
            from signal_engine import get_ai_comment
            ai_comment = await get_ai_comment(best_signal)

            # Форматируем
            sig_text = format_signal(best_signal, ai_comment=ai_comment)

            # Добавляем новости перед сигналом
            if news_text:
                full_text = news_text + "\n\n" + "─" * 28 + "\n\n" + sig_text
            else:
                full_text = sig_text
        ...
        teaser = (
            ...
            + f"\n\n{'─' * 28}\n"
            f"_Полный сигнал с уровнями, стопом и целью — по подписке_"
        )
