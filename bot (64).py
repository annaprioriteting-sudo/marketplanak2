...
from config import (
    ...
    CRYPTO_SYMBOLS_ONLY, ADMIN_IDS, PAYMENT_INFO,
    FREE_DAILY_SIGNALS,
)
...
# ══════════════════════════════════════════════════════════
#  ЕДИНОЕ хранилище анализов — используется везде в боте
#  и в планировщике через BotProxy
# ══════════════════════════════════════════════════════════
...
async def run_analysis(symbol: str, asset_type: str) -> str:
    ...
    try:
        ...
        # ── Сохраняем в общий словарь (доступен и планировщику)
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "🌙 *21:00* — итог дня\n\n"
        "Рынки: крипто-фьючерсы\n"
        "Метод: Smart Money Concepts\n\n"
        "👇 Нажми *📊 Отчёт* для загрузки данных",
...
async def cmd_mystatus(message: Message, state: FSMContext):
    ...
    level = get_access_level(message.chat.id)
    extra = "\n\nИспользуй /upgrade чтобы получить полный доступ." if level == "free" else ""
    await message.answer(
        f"👤 *Твой статус:*\n\n{info}{extra}",
...
async def _show_signals(chat_id: int):
    ...
    try:
        ...
        # Обновляем статус активных сигналов
        for symbol, fa in active_analyses.items():
            if fa.current_price > 0:
                from signal_engine import update_signal_status
                update_signal_status(symbol, fa.current_price)
...
async def fsm_got_symbol(message: Message, state: FSMContext):
    ...
    text = await run_analysis(symbol, "crypto")
    await safe_send(message.chat.id, f"◆  {text}")
...
async def main():
    ...
    class BotProxy:
        """
        Прокси-объект для планировщика.
        Читает и пишет напрямую в глобальный active_analyses.
        """
        @property
        def active_analyses(self):
            return active_analyses
        ...
        def active_analyses(self, val: dict):
            # Обновляем глобальный словарь на месте — все ссылки остаются живыми
            active_analyses.clear()
            active_analyses.update(val)
